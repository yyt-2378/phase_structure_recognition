...
class DCVAESRUnet(nn.Module):
    def __init__(self, sr_args, vae_args):
        super(DCVAESRUnet, self).__init__()
    ...
    def forward(self, img: torch.Tensor, label: torch.Tensor):
        ...
        # adjust vae model output to [0, 1]
        unet_input = self.mean_shift(sr_output)
        # step3: Unet segmentation
        output = self.Unet_segm(unet_input)

        return [vae_output_noise, vae_output, sr_output, output]

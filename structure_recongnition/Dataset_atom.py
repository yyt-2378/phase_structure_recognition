...
img_size = 64
# 封装，对后面读取的图片的格式转换
tran = Trans.Compose([Trans.ToTensor()])
...
class StemDataset(data.Dataset):
    def __init__(self, mode, dir):
        ...
        if self.mode == 'train':
            train_img_dir = dir + '/training/label/'  # 更新地址
            train_label_dir = dir + '/training/catalyst_label/'
            ...
            val_img_dir = dir + '/validation/label/'  # 更新地址
            val_label_dir = dir + '/validation/catalyst_label/'
    ...
    def __getitem__(self, item):  # 获取数据
        if self.mode == 'train':
            ...
            label_y = Image.open(self.img_label[item]).convert('L')
            label_y = np.array(label_y)
            label_y = np.where(label_y > 1, 1, label_y)
            label_tensor = torch.tensor(label_y, dtype=torch.int64)
